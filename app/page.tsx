...
export default function Home() {
  ...
  const shownEntries = useMemo(() => entries.filter((entry) => (screen === "recebimentos" ? entry.type === "recebimento" : screen === "pagamentos" ? entry.type === "pagamento" : true) && (filterMethod === "Todos" || entry.method === filterMethod) && (filterDestination === "Todos" || entry.destination === filterDestination)).sort((a, b) => b.date.localeCompare(a.date) || b.id.localeCompare(a.id)), [entries, screen, filterMethod, filterDestination]);
  ...
  useEffect(() => {
    ...
    async function loadEntries() {
      if (!supabase) { if (active) { setDatabaseStatus("missing"); setDatabaseMessage("Conecte as variáveis do Supabase para salvar os lançamentos."); } return; }
  ...
  return <main className="app-shell">
    <aside className="sidebar">
      ...
      <nav aria-label="Navegação principal">
        ...
        <NavButton active={screen === "graficos"} icon="⌁" label="Gráficos" onClick={() => setScreen("graficos")} />
      </nav>
    ...
    <section className="workspace">
      <header><div><p className="eyebrow">FINANCEIRO</p><h1>{pageTitle[screen]}</h1></div><button className="period">{formatDate(entryDate)} <span>⌄</span></button></header>
      ...
      {screen === "saldo" && <><section className="balance-grid"><BalanceCard label="Caixa" value={totals.Caixa} detail="Movimentações em dinheiro" /><BalanceCard label="Banco" value={totals.Banco} detail="Pix e cartão" /><BalanceCard total label="Saldo disponível" value={total} detail="Caixa + Banco" /></section><section className="section-head section-action"><div><h2>Movimentações do período</h2><p>Todos os recebimentos e pagamentos registrados.</p></div><ExportButton entries={shownEntries} title="saldo" /></section><EntryTable entries={shownEntries} onEdit={requestEdit} /></>}
      {(screen === "recebimentos" || screen === "pagamentos") && <><section className="list-top"><div><h2>{screen === "recebimentos" ? "Entradas registradas" : "Pagamentos registrados"}</h2><p>Consulte, filtre e edite suas movimentações.</p></div><div className="list-actions"><ExportButton entries={shownEntries} title={screen} /><button className="new-entry" onClick={() => setScreen("lancamentos")}>＋ Novo lançamento</button></div></section><section className="filters"><span>Filtrar por</span><select value={filterMethod} onChange={(event) => setFilterMethod(event.target.value)}><option>Todos</option>{methods.map((value) => <option key={value}>{value}</option>)}</select><select value={filterDestination} onChange={(event) => setFilterDestination(event.target.value)}><option>Todos</option><option>Caixa</option><option>Banco</option></select><button onClick={() => { setFilterMethod("Todos"); setFilterDestination("Todos"); }}>Limpar filtros</button></section><EntryTable entries={shownEntries} onEdit={requestEdit} /></>}
      {screen === "graficos" && <Charts entries={entries} period={chartPeriod} onPeriodChange={setChartPeriod} />}
    </section>
    {editing && <EditDialog entry={editing} unlocked={unlocked} password={password} error={passwordError} onPasswordChange={setPassword} onVerify={verifyPassword} onClose={() => setEditing(null)} onSave={saveEdit} />}
  </main>;
}
...
function Charts({ entries, period, onPeriodChange }: { entries: Entry[]; period: "7" | "30" | "all"; onPeriodChange: (period: "7" | "30" | "all") => void }) {
  const sales = entries.filter((entry) => entry.type === "recebimento");
  const dates = useMemo(() => periodDates(sales, period), [sales, period]);
  ...
  return <><section className="list-top chart-heading"><div><h2>Vendas no tempo</h2><p>Acompanhe as entradas registradas no período escolhido.</p></div><select className="chart-period" value={period} onChange={(event) => onPeriodChange(event.target.value as "7" | "30" | "all")}><option value="7">Últimos 7 dias</option><option value="30">Últimos 30 dias</option><option value="all">Todo período</option></select></section><section className="chart-card"><h2>Vendas por método</h2><LineChart dates={dates} series={series} emptyText="Ainda não há recebimentos neste período." /></section><section className="chart-card"><h2>Vendas totais</h2><LineChart dates={dates} series={[{ label: "Total", values: totalValues }]} emptyText="Ainda não há vendas neste período." /></section></>;
}

function periodDates(entries: Entry[], period: "7" | "30" | "all") { const dates = [...new Set(entries.map((entry) => entry.date))].sort(); if (period === "all") return dates; const cutoff = new Date(); cutoff.setDate(cutoff.getDate() - (Number(period) - 1)); return dates.filter((date) => date >= cutoff.toISOString().slice(0, 10)); }
